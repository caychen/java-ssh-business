...
public class PoiUtil {
	...
	// 指定图片类型为jpg
	public void setPicture(HSSFWorkbook wb, HSSFPatriarch patriarch, String pic, int iRow, int iCol)
			throws IOException {
		// 判断文件是否存在
		...
		if (imgFile.exists()) {
			...
			HSSFClientAnchor anchor = new HSSFClientAnchor(190, 0, 1000, 0, (short) (iCol), iRow - 1,
					(short) (iCol + 1), iRow);
	...
	// 指定图片类型为jpg
	public void setPicture(HSSFWorkbook wb, HSSFPatriarch patriarch, String pic, int iRowStart, int iColStart,
	...
	// 画线
	public void setLine(HSSFWorkbook wb, HSSFPatriarch patriarch, int iRowStart, int iColStart, int iRowStop,
			int iColStop) {
	...
	// 计算行高度，实现行自动适应高度 defaultRowHeight = 12.00f; //每一行的高度指定
	...
	public HSSFCellStyle whiteBox(HSSFWorkbook wb) {
		...
		curStyle.setTopBorderColor(HSSFColor.HSSFColorPredefined.WHITE.getIndex());
		curStyle.setRightBorderColor(HSSFColor.HSSFColorPredefined.WHITE.getIndex());
		curStyle.setBottomBorderColor(HSSFColor.HSSFColorPredefined.WHITE.getIndex());
		curStyle.setLeftBorderColor(HSSFColor.HSSFColorPredefined.WHITE.getIndex());
