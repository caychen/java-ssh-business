...
import org.com.cay.utils.file.PoiUtil;
...
public class ContractPrint {
	...
	public void print(Contract contract, String path, HttpServletResponse response) throws Exception {
		...
		List<ContractProduct> cList = new ArrayList<ContractProduct>(oList);
		
		// 填写每页的内容，之后在循环每页读取打印
		Map<String, String> pageMap = null;
		List<Map<String, String>> pageList = new ArrayList<Map<String, String>>(); // 打印页
		...
		for (int i = 0; i < oList.size(); i++) {
			...
			oProduct = cList.get(i);
			pageMap = new HashMap<String, String>(); // 每页的内容
		...
		for (int p = 0; p < pageList.size(); p++) {
			...
			nCell = nRow.createCell(5);
			nCell.setCellType(CellType.NUMERIC);
			...
			if (UtilFuns.isNotEmpty(printMap.get("Cnumber")) && UtilFuns.isNotEmpty(printMap.get("Price"))) {
				nCell.setCellType(CellType.FORMULA);
